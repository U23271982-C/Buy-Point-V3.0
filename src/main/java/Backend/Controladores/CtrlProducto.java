        ...
        try (CallableStatement comando = SQLServerConexion.instanciaConexcion()
            ...
            comando.setInt(2,
                    nuevaEntidad.getLote()
                            .getCantidad());
            comando.setDate(3, java.sql.Date.valueOf(nuevaEntidad.getLote()
                    .getFechaCaducidad()));
            comando.setString(4, nuevaEntidad
                    .
                    getNombreProducto());
            comando.setString(5, nuevaEntidad.getDescripcion());
            comando.setString(6, nuevaEntidad.getEmpaque()
                    .getTipoEmpaque());
            comando.setString(7, nuevaEntidad
                    .getCategoriaProducto().getNombre());
            comando.setBigDecimal(8, nuevaEntidad.getPrecio());
            comando.setBigDecimal(9, nuevaEntidad.getPrecioCosto());

            comando.executeUpdate();
            String aviso = String.format("Se registró el Producto: %s",
                    nuevaEntidad.getNombreProducto());
            JOptionPane.showMessageDialog(null,
                    String.format("Se registró el producto: %s",
                            nuevaEntidad.getNombreProducto()));
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, String.format("ERROR al registrar el producto: %s",
                    nuevaEntidad.getNombreProducto()));
        }
    }
...
public Producto leerPorNombre(String nombreProducto) {
    Producto producto = null;
    String consultaSQL = "{ CALL pa_leerProductoPorNombre(?) }";

    try (CallableStatement comando = SQLServerConexion.instanciaConexcion()
            .getConnection().prepareCall(consultaSQL)) {

        comando.setString(1, nombreProducto);
        ResultSet filas = comando.executeQuery();
        ...
        if (filas.next()) {
            ...
            producto.setPrecio(filas.getBigDecimal(5));
            producto.setPrecioCosto(filas.getBigDecimal(6));
            producto.getInventario().setStock(filas.getInt(7));
            producto.getCodigo().setCodigo(filas.getString(8));
        }

    } catch (SQLException e) {
        System.out.println("Error al leer producto por nombre: " + e.getMessage());
    }
    ...
    public void actualizar(Producto actualizarEntidad) {
        ...
        try (CallableStatement comando = SQLServerConexion.instanciaConexcion()
            ...
            comando.executeUpdate();
            System.out.println("Se realizó la lectura");
            JOptionPane.showMessageDialog(null, "Se actulizo las Credenciales");
